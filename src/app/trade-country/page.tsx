// src/app/trade-country/page.tsx
"use client";

import { useState } from "react";
import { Filter, Search } from "lucide-react";
import countries from "@/data/countries.json"; // ← countries.json замаа өөрийн structure-т тааруул
...
export default function TradeCountryPage() {
  ...
  const [selectedCountry, setSelectedCountry] = useState<string>(""); // country_code
  ...
  async function handleFetch() {
    ...
    if (selectedCountry) {
      // Backend талд хэрвээ параметрын нэр өөр бол (ж: country=)
      // зөвхөн эндээс сольж өгнө.
      params.set("senderReceiver", selectedCountry);
    }
    params.set("page", "1");
  ...
  // --- CSV export: бүх хуудсуудыг татаж CSV болгоно ---
  async function handleExportCsvFull() {
    ...
    try {
      ...
      if (selectedMonth) params.set("month", selectedMonth);
      if (selectedCountry) {
        // Хэрвээ backend дээр country гэсэн нэртэй байвал энд соль
        params.set("senderReceiver", selectedCountry);
      }
      params.set("page", "1");
      ...
      const c = selectedCountry || "all";
      a.href = urlBlob;
      a.download = `foreign_trade_full_${y}_${m}_${c}.csv`;
  ...
  const selectedCountryName =
    selectedCountry &&
    countryOptions.find((c) => c.country_code === selectedCountry)?.country_name;
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-4 space-y-8">
        ...
        <header className="space-y-2">
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">
            Гадаад худалдаа – улс / сар / жилээр
          </h1>
          <p className="text-sm text-slate-400 max-w-2xl">
            Жил, сар, улс сонгож гадаад худалдааны (импорт / экспорт) өгөгдлийг
            API-аас татаж хүснэгтээр харна.
          </p>
        </header>
